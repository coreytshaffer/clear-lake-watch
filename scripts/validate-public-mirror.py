...
def assert_manifest_freshness(
    ...
    field_definitions = manifest.get("fieldDefinitions")
    if not isinstance(field_definitions, dict):
        state.fail("Manifest must include fieldDefinitions for trust clarity.")
    else:
        for field_name in (
            "generatedAt",
            ...
            if not isinstance(definition, str) or not definition.strip():
                state.fail(f"Manifest fieldDefinitions is missing {field_name}.")

    freshness_legend = manifest.get("freshnessLegend")
    if not isinstance(freshness_legend, list) or len(freshness_legend) < 3:
        state.fail("Manifest must include a freshnessLegend with at least three entries.")
    else:
        legend_text = " ".join(
            str(entry.get("term", "")) + " " + str(entry.get("definition", ""))
            for entry in freshness_legend
        ...
        for required_phrase in (
            ...
            if required_phrase not in legend_text:
                state.fail(f"Manifest freshnessLegend is missing {required_phrase}.")
    ...
    for source in sources:
        ...
        if source_id.startswith("fhabs-"):
            resource_date = source.get("resourceDate")
            resource_age_days = source.get("resourceAgeDays")
            if not resource_date:
                state.fail(f"Manifest source {source_id} is missing resourceDate.")
            else:
                try:
                    resource_datetime = parse_date_like(str(resource_date))
                except ValueError:
                    resource_datetime = datetime.fromisoformat(
                        f"{resource_date}T00:00:00"
                    )
                if resource_datetime.date() > manifest_generated_at.date():
                    state.fail(
                        f"Manifest source {source_id} resourceDate is after "
                        "the manifest generatedAt."
                    )
            if not isinstance(resource_age_days, (int, float)) or resource_age_days < 0:
                state.fail(
                    f"Manifest source {source_id} must have non-negative resourceAgeDays."
                )

    notes_text = " ".join(str(note) for note in manifest.get("notes", []))
    for required_note in (
        ...
        if required_note not in notes_text:
            state.fail(f"Manifest notes are missing required trust note: {required_note}")
